...
impl Stack {
    fn settle_pass(&mut self) -> std::collections::HashSet<usize> {
    ...
    fn settle(&mut self) -> std::collections::HashSet<usize> {
        let mut fallen_bricks = std::collections::HashSet::new();

        loop {
            let newly_fallen = self.settle_pass();
            if newly_fallen.len() == 0 {
                return fallen_bricks;
            } else {
                fallen_bricks.extend(newly_fallen.iter());
            }
        }
    }

    fn non_supporting_bricks(&self) -> usize {
        ...
        for brick_idx in 0..self.bricks.len() {
            ...
            let fallen_bricks = jenga_stack.settle().len();
...
fn day22part1(input: &str) -> usize {
    ...
    println!("settled");
    stack.visualize();
    return stack.non_supporting_bricks();
}
